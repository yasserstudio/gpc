...
describe("secretsScanner", () => {
  ...
  it("detects AWS access keys", async () => {
    await writeFile(join(tmpDir, "config.ts"), `
  ...
  it("detects Stripe secret keys", async () => {
    await writeFile(join(tmpDir, "Payment.java"), `
  ...
  it("detects private keys", async () => {
    await writeFile(join(tmpDir, "key.properties"), `
  ...
  it("detects generic API tokens", async () => {
    await writeFile(join(tmpDir, "config.kt"), `
  ...
  it("skips build directory", async () => {
    ...
    await writeFile(join(buildDir, "config.json"), `{
  ...
  it("scans nested directories", async () => {
    ...
    await writeFile(join(nested, "Config.kt"), `
...
line4`);
  ...
  it("detects multiple secrets in same file", async () => {
    await writeFile(join(tmpDir, "multi.kt"), `
