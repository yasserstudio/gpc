...
This is a deepsec scanning workspace. Project `GPC` was just registered
(target: `..`). Setup is incomplete — `data/GPC/INFO.md`
...
2. **Fill in `data/GPC/INFO.md`.** It's auto-injected into the AI
...
```bash
pnpm deepsec scan    --project-id GPC
pnpm deepsec process --project-id GPC
```
