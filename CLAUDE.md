...
- Current version: v0.9.95 — pre-release series (`0.9.x` → `1.0.0` public launch)
- v0.9.95: global-flag collision fix + profile safety (GH #103). Commander resolves a subcommand flag that shares a name with a root global (`-p/--profile`, `-a/--app`, `-y/--yes`, `--dry-run`, `-o/--output`, `--notify`) to the ROOT option, so the subcommand never saw it — broken since v0.8.3, surfaced when softlion's `auth login --profile` created no profile. Fallback reads (`options.x ?? program.opts()`) for same-semantic flags (auth login/logout `--profile`, init `--app`, install-skills `--yes`, migrate dry-run via `isDryRun`); renames for never-worked flags: reviews export `--output-file`, migrate fastlane `--out-dir`, status `--desktop-notify`, generated-apks/system-apks download `--output-file` (both had a required `--output` that could never be satisfied). Review-driven hardening: `setProfileConfig` merges instead of replacing (key rotation keeps `app`/`developerId`/`reports`), `auth logout --profile` clears credentials only via new `clearProfileCredentials` (entry + dangling active-profile pointer removed only when nothing else remains), `deleteProfile` clears dangling pointers, `doctor --fix` refuses to overwrite an existing-but-unloadable config, `--adc --profile` is a usage error, wizard defaults its profile prompt to a global `-p`, `runWatchLoop` gained `onStatus` so `status --watch --desktop-notify` actually notifies. Also: docs deploy had been red since the deepsec-retirement commit (unfenced `${{ }}` in github-actions.md crashed Vue) and homepage coverage cards 404'd (raw hrefs missing the `/gpc/` base — now `withBase()`). Lesson: command tests that build synthetic roots with partial option sets cannot catch this class — the regression suite drives the real `createProgram()`. 2,630 tests.
- v0.9.94: honest upload errors + plugin permission enforcement. Play returns a plain 403 when a Play Console "App content" declaration is incomplete, and its text often contains the word "permissions" (the Foreground Service gate reads "...uses any Foreground Service permissions"); the 403 classifier matched a bare `permission` substring and replaced Google's explanation with "the service account does not have permission" (GH #101, cost the reporter ~5h). New `API_DECLARATION_REQUIRED` code ordered before the permissions branch, quoting Google's message verbatim; permissions branch narrowed to explicit denial phrasing, which also stops insufficient-OAuth-scope 403s being misreported. Preventive half: preflight `policy-app-content-declaration` advisory (info) when the AAB requests any `FOREGROUND_SERVICE*` permission. Plugin trust moved from name prefix (`specifier.startsWith("@gpc-cli/")`, which never verified the package was ours) to resolved manifest identity, verified _before_ `import()`; third-party plugins must declare `gpc.permissions`. Plugin request/response/onError hooks were dead code and are now wired to the real transport with credential redaction. Breaking (plugin authors): undeclared third-party plugins are refused; relative-path approvals need re-approving. 2,612 tests.
- v0.9.93: live GCS report download. `gpc reports list` / `download stats` / `download financial` now fetch Play bulk-report CSVs directly from the account's Cloud Storage bucket (`pubsite_prod_<developerId>`) — they previously errored as unavailable. Discovery-first GCS layer (list prefix → digit-boundary month match → download → gunzip + UTF-16→UTF-8 decode), financial ZIP unwrap (single CSV auto, multi via `--json` or `--output-file report.zip`), new `--bucket`/`--dimension` flags + `reports.bucket` config + `GPC_REPORTS_BUCKET` env, `gpc doctor` reports-bucket probe, new `gpc auth clear-cache`. Least-privilege auth: `devstorage.read_only` scope is requested only by the reports path (scope-keyed token cache); setup requires the "download bulk reports (read-only)" grant for the SA in Play Console (not automatic). Breaking (SDK only): api drops the fictional `client.reports.list` + `ReportBucket`/`ReportsListResponse`; core replaces `downloadReport` with `downloadStatsReport`/`downloadFinancialReport`. 2,540 tests.
- v0.9.92: screenshot display-order fix for `listings images sync --delete`. When a language/type differs from local in content or order, the whole combo is cleared with a single `images.deleteAll` (fewer Play API write calls) and re-uploaded in filename order; a set already in order is skipped. Safety: an absent local directory is no longer read as "delete everything", so syncing a subset of image types never wipes a remote icon/feature graphic (a present-but-empty directory still clears explicitly). Contributor fix by softlion (PR #97) + maintainer hardening. Dependabot now holds deferred majors (commander 15, @ai-sdk 4, typescript 6, vite 8, ai 7) until post-1.0.0. 2,503 tests.
...
- 2,630 total tests, 7 packages building, 90%+ line coverage on all core packages
...
- Vitest for all tests (2,630 total across 7 packages + e2e)
