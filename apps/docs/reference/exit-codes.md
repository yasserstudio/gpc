...
| `3`  | Auth / Missing Credentials | No service-account credentials, expired token, invalid key, revoked access; or `gpc changelog generate --ai` with no provider key in env | Re-authenticate or set the required provider env var |
...
When a locale fails at runtime (without `--strict`), GPC classifies the cause into one of these short reason slugs so sensitive error strings never reach your Play Store draft or JSON output:

| Reason           | Cause                                                                |
| ---------------- | -------------------------------------------------------------------- |
| `rate_limited`   | 429 rate limit (or Anthropic 529 Overloaded). Retry with backoff.    |
| `auth`           | 401/403 or invalid-key error. Check your provider key.               |
| `safety_blocked` | Content filter rejected the prompt (Google `SAFETY`, OpenAI policy). |
| `timeout`        | AbortError / TimeoutError / >30s elapsed.                            |
| `network`        | DNS, connection refused, ETIMEDOUT, or generic fetch error.          |
| `no_source`      | Source locale missing from the locale list; translation skipped.     |
| `unknown`        | Unrecognized error shape. Enable `GPC_DEBUG=1` to inspect stderr.    |
