...
GPC v0.9.60 does **not** have a single "sync all images from a directory" command. `gpc listings push` handles text metadata only. For bulk image upload, you need to loop over your files and call `gpc listings images upload` per file. The pattern below works and is what users should run today. A future `gpc listings images push --dir <path>` is an open feature request — if you want it, open an issue on [github.com/yasserstudio/gpc](https://github.com/yasserstudio/gpc/issues).
:::
