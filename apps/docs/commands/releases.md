...
| `--since`                       |       | `string` |             | Git ref to start from (tag, SHA) — used with `--notes-from-git` or `--changelog-ai`                                                                                         |
| `--changelog-ai`                |       | flag     |             | Generate AI-translated Play Store release notes from git history, using your own LLM key. See [`gpc changelog generate`](./changelog#changelog-generate).                   |
| `--locales`                     |       | `string` |             | Locales for `--changelog-ai`: comma-separated BCP 47 codes or `auto` (translate every listing locale).                                                                      |
| `--provider`                    |       | `string` |             | AI provider for `--changelog-ai` (`anthropic`, `openai`, `google`).                                                                                                         |
| `--model`                       |       | `string` |             | Override the AI model used for `--changelog-ai`.                                                                                                                            |
