...
- [x] **Games API enhancements** (Track E) — E1 achievement/leaderboard **icon upload** (`gpc games … set-icon`, new `uploadGamesImage` media path) and E3 **bulk config sync** (`gpc games … push/pull <dir>`, create-or-update by id, `--dry-run`, pagination) shipped. **E2 (Games Management reset) deferred to post-1.0** — the reset/hide ops need 3-legged player-consent OAuth (games scope), which GPC's service-account auth cannot provide (document-rather-than-ship-broken). 2-agent review, security clean.
